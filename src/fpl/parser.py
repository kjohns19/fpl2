...
class Parser:
    ...
    def __parse_if(self, tokenization, index):
        ...
        return fpl.syntax.NodeIf(condition, ifnode, elsenode), index
    ...
    def __parse_fun(self, tokenization, index):
        code, index = self.__parse(tokenization, index=index, stop=['end'])
        return fpl.syntax.NodeFunction(code), index
